...
const validateSession = async(req, res, next) => {
    ...
    if (req.method == "OPTIONS") {
        //above determines if safe to send (you need a token for any of the HTTP reqs) then next moves us onto next part of request
        next();
    } else if (
        ...
        if (payload) {
            //sequelize method of findOne to look for user in our User Model where the ID of the user in the database
